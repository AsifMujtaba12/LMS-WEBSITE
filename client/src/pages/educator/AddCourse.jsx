...
import uniqid from 'uniqid';  // unique id for each course
...
const AddCourse = () => {
  ...
  const editorRef= useRef(null); // course description
...
const handleSubmit = async (e)=>{
  ...
  const courseData = {
    ...
    courseDescription: quillRef.current.root.innerHTML, //  Rich text from Quill editor
    coursePrice: Number(coursePrice), // Convert course price to number
    discount: Number(discount), //  Convert discount to number
    courseContent: chapters, // Array of chapters
  };
  const formData = new FormData(); //Create a FormData object (used to send file + data together)
  formData.append('courseData', JSON.stringify(courseData)); //Add stringified course data
  formData.append('image', image); //  Add selected image file
  ...
  if(data.success){
    toast.success(data.message)
      //Reset the form fields after successful course addition
    setCourseTitle('');
    setCoursePrice(0);
    setDiscount(0);
    setImage(null);
  ...
  return (
    ...
    className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
      <div className='flex flex-col gap-1'>
        ...
        value={courseTitle} type='text' placeholder='Type here' className='outline-none md:py-2.5
      ...
      <div className='flex items-center justify-between flex-wrap mt-20'>
        <div className='flex flex-col gap-1'>
          ...
          <input onChange={e=>setCoursePrice(e.target.value)} value={coursePrice}
            type='number' placeholder='0' className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500'
            required />
        </div>
        <div className='flex md:flex-row flex-col items-center gap-3.5'> 
      ...
      <div>
        {chapters.map((chapter, chapterIndex)=>(
          <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
            <div className='flex justify-between items-center p-4 border-b'>
              ...
              src={assets.dropdown_icon} width={14} alt='' className={`mr-2 cursor-pointer transition-all ${chapter.collapsed &&
              "-rotate-90"}`}/>
            ...
            {/* lecturs avialable when no cloapsed */}
            {!chapter.collapsed && (
              <div>
                {chapter.chapterContent.map((lecture, lectureIndex)=>(
                 ...
                 <img onClick={()=>handleLecture('remove', chapter.chapterId, lectureIndex)}
        ...
        onClick={()=> handleChapter('add')}>+ Add Chapter</div>
          ...
          showPopup && (
            ...
            <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
              ...
              <div className='flex gap-2 my-4'>
                <p>Is Preview Free?</p>
                <input type='checkbox' className='mt-1 block w-full border rounded py-1 px-2'
